...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def _build_message_event(
        ...
        # Try to resolve type exactly as the previous code did
        telegram_chat_type = str(getattr(chat, "type", "")).split(".")[-1].lower()
        if "chattype." in telegram_chat_type:
            telegram_chat_type = telegram_chat_type.replace("chattype.", "")
        if "supergroup" in telegram_chat_type or "group" in telegram_chat_type:
